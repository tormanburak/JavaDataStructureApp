...
public class BinaryTree {
	...
	{ // (assumes non-empty tree)
		...
		while (current.date != key) // while no match;
		{
			if (key < current.date) // go left ?
			{
	...
	public boolean delete(int key) { // delete node with given key
		...
		while (current.date != key) { // search for key
			parent = current;
			if (key < current.date) { // go left
				isLeftChild = true;
				current = current.leftChild;
